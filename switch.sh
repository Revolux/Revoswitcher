...
# (PIDFILE=$(mktemp /tmp/foo.XXXXXX) && trap "rm $PIDFILE" 0 \
         # && { (unbuffer ${CURDIR}/zcash/miner --eexit 3 --intensity 64 --cuda_devices 0 1 2 3 4 5 6 --templimit 80 --server btcz.suprnova.cc --port 5587 --user ak.${NAKE} --pass x) \
...
#Equihash
(PIDFILE=$(mktemp /tmp/foo.XXXXXX) && trap "rm $PIDFILE" 0 \
         && { (unbuffer ${CURDIR}/zcash/miner --eexit 3 --intensity 64 --cuda_devices 0 1 2 3 4 5 6 --templimit 80 --server btg.suprnova.cc --port 8816 --user ak.${NAME} --pass x) \
                  1> >(tee >(grep -q "Total speed: 0 Sol/s" && kill $(cat $PIDFILE)) >&1) \
              & PID=$! && echo $PID >$PIDFILE ; wait $PID || true; }) 1> >(tee >(grep -q "ERROR: unknown error" && (echo 'revolux123' | sudo -S reboot) >&1))
done
